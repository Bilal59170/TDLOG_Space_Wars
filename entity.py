...
class Entity:
    ...
    @property
    def pos(self):
        return np.array([self._pos[0] % self.map.size[0], self._pos[1] % self.map.size[1]])
    @property
    def map_pos(self):
        return self.pos
    @property
    def screen_pos(self):
        ...
        pos_0 = (self.pos - self.map.center).astype(int)                                            # Position 1 : Position normale
        pos_1 = (self.pos - self.map.center + self.map.size).astype(int)                            # Position 2 : A gauche ou en bas
        x = pos_0[0] if abs(pos_0[0] - WIN_SIZE[0]/2) < abs(pos_1[0] - WIN_SIZE[0]/2) else pos_1[0]
        y = pos_0[1] if abs(pos_0[1] - WIN_SIZE[1]/2) < abs(pos_1[1] - WIN_SIZE[1]/2) else pos_1[1]
