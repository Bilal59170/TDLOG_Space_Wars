...
def get_sprite_mask(sprite):
    ...
    if img_data.format == "RGBA":
        ...
        alpha = (
            np.frombuffer(i, dtype=np.uint8)[::4]
            .astype(bool)
            .reshape(sprite.height, sprite.width)[::BITMAP_RATIO, ::BITMAP_RATIO]
        )
    else:
...
class BitmapSprite(Entity, Sprites):
    def __init__(
        ...
        if game_state is not None:
            self.sprite = pyglet.sprite(
                image, x=Entity.screen_x, y=Entity.screen_y, batch=game_state.batch
            )
            self.inBatch = True
        else:
    ...
    def intersects(self, other):
        if not (
            self.screen_x < other.screen_x + other.sprite.width
            and self.screen_x + self.sprite.width > other.screen_x
            and self.screen_y < other.screen_y + other.sprite.height
            and self.screen_y + self.sprite.height > other.screen_y
        ):
